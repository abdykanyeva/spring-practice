...
public class CourseController_ResponseWrapper {
    ...
    public ResponseEntity<ResponseWrapper> getAllCourses(){
        return ResponseEntity.status(HttpStatus.ACCEPTED)
                ...
                .body(new ResponseWrapper("Courses retrieved successfully", courseService.getCourses()));
    }


    @GetMapping("{id}")
    public ResponseEntity<ResponseWrapper> getCourseById(@PathVariable("id") Long id){
        return ResponseEntity.ok(new ResponseWrapper("Course retrieved by id", courseService.getCourseById(id)));
    }
