...
public interface AccountRepository extends JpaRepository<Account, Long> {
    ...
    List<Account> findByAgeBetween(int age1, int age);
    ...
    List<Account> retrieveAdminAccount(UserRole admin);

    //Write a JPQL query to sort all accounts with age

    @Query("SELECT e FROM Account e ORDER BY e.age")
    List<Account> sortAllAccountsWithAge(int age);
    ...
    @Query(value = "SELECT * FROM account_details WHERE age < value", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM account_detail WHERE age > val", nativeQuery = true)
    List<Account> readAllAccountsHigher(int val);
