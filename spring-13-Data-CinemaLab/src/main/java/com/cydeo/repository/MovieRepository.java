...
public interface MovieRepository extends JpaRepository<Movie, Long> {
    ...
    @Query("SELECT e FROM Movie e WHERE e.price BETWEEN ?1 AND ?2")
    List<Movie> listAllMovieBetweenRangeOfPrices(@Param("price1")BigDecimal p1, @Param("price2")BigDecimal p2);


    //Write a JPQL query that returns all movie names
    @Query("SELECT DISTINCT m.name FROM Movie m")
    List<String> retrieveMovieName(String name);
    ...
    List<Movie> retrieveAllMoviesBetweenTheRangeOfPrice(@Param("price1")BigDecimal p1, @Param("price2")BigDecimal p2);
    ...
    @Query(value="SELECT * FROM movie ORDER BY price DECSC LIMIT 5", nativeQuery = true)
