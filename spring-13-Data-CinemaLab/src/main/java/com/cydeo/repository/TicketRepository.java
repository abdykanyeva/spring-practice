...
public interface TicketRepository extends JpaRepository<Ticket, Long> {
    ...
    List<Ticket> findAllByDateTimeBetween(LocalDateTime d1, LocalDateTime d2);
    ...
    List<Ticket> retrieveAllTicketByUseAccount(@Param("userId") Long userId);

    //Write a JPQL query that returns all tickets between a range of dates

    @Query("SELECT t FROM Ticket t WHERE t.dateTime BETWEEN ?1 AND ?2")
    List<Ticket> fetchAllTicketsBetweenRangeOfDates(@Param("dateTime1")LocalDateTime dateTime1, @Param("dataTime2") LocalDateTime dateTime2);
    ...
    @Query(value="SELECT count(*) FROM ticket WHERE user_account_id = ?1 AND date_time BETWEEN ?2 AND ?1", nativeQuery = true)
    ...
    List<String> retrieveAllDistinctMove();
    ...
    @Query(value="SELECT * FROM ticket t JOIN user_account ua ON t.user_account_id = ua.id"+
            ...
            "JOIN movie_cinema mc ON mc.id = t.movie_cineam_id"+
            "JOIN movie m ON mc.movie_id = m.id"+
            "WHERE ua.username ILIKE concat", nativeQuery = true)
